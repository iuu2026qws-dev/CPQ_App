...
export const usePricingStore = defineStore('pricing', () => {
  ...
  const loadEntries = async (priceBookId: number) => { entryLoading.value = true; try { const data = await getEntryList({ priceBookId }); entries.value = Array.isArray(data) ? data : [] } finally { entryLoading.value = false } }

  // ===== 定价规则 =====
  const rules = ref<CpqPriceRule[]>([])
  const ruleLoading = ref(false)
  const loadRules = async () => { ruleLoading.value = true; try { const data = await getPriceRuleList(); rules.value = Array.isArray(data) ? data : [] } finally { ruleLoading.value = false } }

  // ===== 阶梯定价 =====
  const tiers = ref<CpqVolumeTier[]>([])
  const tierLoading = ref(false)
  const loadTiers = async () => { tierLoading.value = true; try { const data = await getVolumeTierList(); tiers.value = Array.isArray(data) ? data : [] } finally { tierLoading.value = false } }

  // ===== 渠道价格 =====
  const channelPrices = ref<CpqChannelPrice[]>([])
  const channelPriceLoading = ref(false)
  const loadChannelPrices = async () => { channelPriceLoading.value = true; try { const data = await getChannelPriceList(); channelPrices.value = Array.isArray(data) ? data : [] } finally { channelPriceLoading.value = false } }

  // ===== 汇率 =====
  const rates = ref<CpqCurrencyRate[]>([])
  const rateLoading = ref(false)
  const loadRates = async () => { rateLoading.value = true; try { const data = await getCurrencyRateList(); rates.value = Array.isArray(data) ? data : [] } finally { rateLoading.value = false } }
