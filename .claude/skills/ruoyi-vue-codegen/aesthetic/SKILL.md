---
name: aesthetic
description: 界面美化-遵循成熟的设计原则来创建美观的界面。适用于构建用户界面/用户体验、从灵感网站分析设计、使用 ai-multimodal 生成设计图像、实现视觉层次结构和色彩理论、添加微交互，或创建设计文档。包括使用 Chrome 开发工具和 ai-multimodal 捕捉和分析灵感截图的工作流程、迭代设计图像生成直至达到美学标准，以及涵盖 BEAUTIFUL（美学原则）、RIGHT（功能性/可访问性）、SATISFYING（微交互）和 PEAK（故事讲述）阶段的全面设计系统指导。与 Chrome 开发工具、ai-multimodal、媒体处理、UI 设计和 Web 框架技能相结合。
---
