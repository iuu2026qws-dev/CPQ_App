...
### 原则 17：admin pom.xml 中 Module 依赖的 `<version>` 标签必须显式声明（2026-06-07 新增）

**铁律**：在 `ruoyi-admin 2/pom.xml` 中为任何子模块添加 `<dependency>` 时，`<version>${revision}</version>` 标签**必须显式写出**，不能省略。即使父 POM 的 `dependencyManagement` 已经声明了版本，Maven 多模块构建中 `<dependency>` 节点仍需要显式声明版本号。

**为什么**：Maven 的 `dependencyManagement` 只在依赖本身的 POM 坐标一致时生效。子模块引用（`<artifactId>ruoyi-cpq-pricing</artifactId>`）如果在 `dependencyManagement` 中没有对应条目，或者 POM 解析顺序导致尚未加载，省略 `<version>` 会导致 Maven 无法确定版本号，编译报错 `'dependencies.dependency.version' is missing`。

**错误现象**：
- 省略 `<version>` → Maven 报错 `'dependencies.dependency.version' for org.dromara:ruoyi-cpq-pricing:jar is missing`
- 重复 `<version>` 标签（两次 `<version>${revision}</version>`）→ Maven 报错 `Duplicate tag: 'version'`
- 正确的写法是恰好一个 `<version>${revision}</version>`

**正确实现**：
```xml
<dependency>
    <groupId>org.dromara</groupId>
    <artifactId>ruoyi-cpq-pricing</artifactId>
    <version>${revision}</version>
</dependency>
```

**反面案例**：Sprint 3 中为 `ruoyi-admin 2/pom.xml` 添加 `ruoyi-cpq-pricing` 依赖时，第一次省略了 `<version>`，导致 `mvn compile` 失败。修复时因手误写了两次 `<version>` 标签，再次编译失败。最终正确写入一次 `<version>${revision}</version>` 后才编译通过。

**教训**：新建 Maven 模块后，在 admin pom.xml 添加依赖是高频操作，不要凭记忆省略 version 标签。建议复制已有模块的 `<dependency>` 块，修改 artifactId 即可。

### 原则 18：MySQL JSON 类型字段在 Java Domain 中用 String 即可，MyBatis-Plus 自动处理序列化（2026-06-07 新增）

**铁律**：表中定义为 `JSON` 类型的字段（如 `condition_json`、`action_json`），在 Java Domain 类中声明为 `private String xxxJson;` 即可，**不需要**添加 `@TableField(typeHandler = ...)` 或任何特殊注解。MyBatis-Plus 的 JDBC 驱动会自动将 Java String 序列化为 MySQL JSON，反之亦然。

**为什么**：MySQL JDBC 驱动（8.0+）内置了对 JSON 类型的支持，`setString()` / `getString()` 原生支持 JSON 列的读写，不需要额外的类型处理器。添加多余的 `@TableField(typeHandler = JacksonTypeHandler.class)` 反而可能导致序列化异常（Jackson 序列化 String 时会额外加引号，产生双引号嵌套）。

**正确实现**：
```java
// Entity / Domain — 表字段 condition_json (MySQL JSON 类型)
@TableField("condition_json")
private String conditionJson;  // 直接用 String，无需额外注解
```

**验证方法**：用 curl POST 测试创建包含 JSON 字段的数据，确认数据库存储的 JSON 格式正确（无多余转义）；再用 curl GET 读取，确认返回的 JSON 字段与写入时一致。
...
- **新建 Maven 模块并添加到 admin pom.xml 依赖**时，必须检查 `<version>` 标签是否显式声明
- **Domain 字段对应 MySQL JSON 类型**时，确认类型为 String 且无多余的 `@TableField(typeHandler=...)` 注解

当前 skill 统计：**18 条原则 + 21 项检查 + 9 个真实案例**
