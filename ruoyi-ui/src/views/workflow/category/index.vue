...
const getList = async () => {
  ...
  try {
    ...
    // 响应拦截器已自动解包 RuoYi { code, data } 包裹体，兼容两种格式
    const arr = Array.isArray(res) ? res : (res as any).data;
    const data = proxy?.handleTree<CategoryVO>(arr, 'categoryId', 'parentId');
    if (data) {
...
const getTreeselect = async () => {
  ...
  // 响应拦截器已自动解包，兼容两种格式
  const arr = Array.isArray(res) ? res : (res as any).data;
  const data = proxy?.handleTree<CategoryOption>(arr, 'categoryId', 'parentId');
  if (data) {
...
const handleUpdate = async (row: CategoryVO) => {
  ...
  // 响应拦截器已自动解包，getCategory 返回的是单对象，直接用 res
