...
const getList = async () => {
  ...
  // 响应拦截器已自动解包 data 字段，res 直接是部门数组
  const data = proxy?.handleTree<DeptVO>(Array.isArray(res) ? res : res.data, 'deptId');
  if (data) {
    deptList.value = data;
  }
  loading.value = false;
...
async function getDeptAllUser(deptId: any) {
  if (deptId !== null && deptId !== '' && deptId !== undefined) {
    ...
    deptUserList.value = Array.isArray(res) ? res : res.data;
  }
}
...
const handleAdd = async (row?: DeptVO) => {
  ...
  const data = proxy?.handleTree<DeptOptionsType>(res, 'deptId');
  if (data) {
...
const handleUpdate = async (row: DeptVO) => {
  ...
  const data = proxy?.handleTree<DeptOptionsType>(response, 'deptId');
  if (data) {
    deptOptions.value = data;
    if (data.length === 0) {
      const noResultsOptions: DeptOptionsType = {
        deptId: res.parentId,
        deptName: res.parentName,
        children: []
      };
