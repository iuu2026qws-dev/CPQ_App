...
const getDeptTree = async () => {
  ...
  enabledDeptOptions.value = filterDisabledDept(treeData);
};
...
onMounted(() => {
  ...
  proxy?.getConfigKey('sys.user.initPassword').then((response) => {
    initPassword.value = Array.isArray(response) ? response : response?.data ?? response;
  });
});

async function handleDeptChange(value: number | string) {
  const response = await optionselect(value);
  postOptions.value = response;
  form.value.postIds = [];
}
</script>
