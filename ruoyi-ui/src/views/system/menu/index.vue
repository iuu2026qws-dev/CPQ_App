...
const getList = async () => {
  ...
  const tempMap = {};
  // 存储 父菜单:子菜单列表
  // 响应拦截器已自动解包 data 字段，res 直接是菜单数组
  const menuData = Array.isArray(res) ? res : (res.data || [])
  ...
  menuList.value = menuData.filter((menu) => !menuIdSet.has(menu.parentId));
  // 根据新数据重新加载子菜单数据
...
const getTreeselect = async () => {
  ...
  menu.children = proxy?.handleTree<MenuOptionsType>(Array.isArray(response) ? response : response.data, 'menuId');
