...
export const usePermissionStore = defineStore('permission', () => {
  ...
  const filterAsyncRouter = (asyncRouterMap: RouteRecordRaw[], lastRouter?: RouteRecordRaw, type = false): RouteRecordRaw[] => {
    return asyncRouterMap.filter((route) => {
      ...
      if (route.children != null && route.children && route.children.length) {
        ...
        // 目录型菜单（无 component）自动 redirect 到第一个可见子菜单
        if (!route.component && !route.redirect) {
          const firstChild = findFirstVisibleChild(route.children);
          if (firstChild) {
            route.redirect = firstChild.path || firstChild.name?.toString();
          }
        }
      } else {
  ...
  /** 递归查找第一个可见子路由 */
  const findFirstVisibleChild = (children: RouteRecordRaw[]): RouteRecordRaw | null => {
    for (const child of children) {
      if (child.meta?.hidden) continue;
      if (child.children && child.children.length > 0) {
        const nested = findFirstVisibleChild(child.children);
